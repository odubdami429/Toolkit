#!/usr/bin/env python3
"""
Pull all Gmail (+ user_accounts, admin) audit logs for a user
and write CSV files for downstream parsing by a Claude skill.
...
Note: the gmail Reports application requires both startTime and
endTime, with a maximum window of 30 days.

Usage:
    python3 pull_gmail_logs.py user@companyDomain [--days 30] [--out logs/]
    python3 pull_gmail_logs.py user@companyDomain --apps gmail
"""
...
DEFAULT_APPS = ["gmail", "user_accounts", "admin"]
...
def main():
    ...
    if args.days > 30 and "gmail" in args.apps:
        sys.stderr.write(
            "[warn] gmail application caps at 30 days; consider chunking longer windows.\n"
        )
    ...
    manifest_path = os.path.join(out_dir, f"{user_slug}_gmail_manifest_{args.days}d.json")
