#!/usr/bin/env python3
"""
Pull OAuth (token) audit logs for a user and write a CSV file for
downstream parsing by a Claude skill.

The Workspace `token` application records every OAuth grant, revoke,
and authorize event for third-party apps acting on behalf of the user.
Useful for spotting new app authorizations, suspicious scopes, or
unexpected client_ids.

Output: one CSV file under --out. Nested parameters (including
messageValue.parameter[]) are flattened into columns. Any extra
parameters not in COMMON_PARAMS are preserved in other_params_json
(lossless).

Usage:
    python3 pull_oauth_logs.py user@companyDomain [--days 30] [--out logs/]
    python3 pull_oauth_logs.py user@companyDomain --apps token
"""
...
def main():
    ...
    for app in [a.strip() for a in args.apps.split(",") if a.strip()]:
        ...
        items, pages = gws_pull(args.user, app, start)
    ...
    manifest_path = os.path.join(out_dir, f"{user_slug}_oauth_manifest_{args.days}d.json")
